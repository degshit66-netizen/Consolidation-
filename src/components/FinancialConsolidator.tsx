...
function UploadZone({ title, entity, onUpload }: { title: string; entity?: any; onUpload: (file: File) => void }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col h-full">
      ...
      <div 
        ...
        className={cn(
          "flex-1 border-2 border-dashed rounded-xl flex flex-col items-center justify-center p-8 transition-all cursor-pointer min-h-[200px]",
          entity?.isValidated ? "border-green-200 bg-green-50" : "border-slate-200 hover:border-blue-400 hover:bg-slate-50"
        )}
      >
        <input 
          type="file" 
        ...
        {entity?.isValidated ? (
          <>
            ...
            <p className="text-sm font-bold text-slate-900 text-center">{entity.fileName || 'File Uploaded'}</p>
            <p className="text-xs text-green-600 mt-1">Trial Balance Balanced (TB=0)</p>
          </>
        ) : (
          <>
            <UploadCloud size={40} className="text-slate-400 mb-4" />
            <p className="text-sm font-medium text-slate-600 text-center">Click to upload Trial Balance CSV</p>
            <p className="text-[10px] text-slate-400 mt-2">Required: Account, Type, Debit, Credit</p>
          </>
        )}
      </div>
      {entity && (
        <div className="mt-4 flex items-center justify-between text-xs pt-4 border-t border-slate-100">
          <div className="flex flex-col">
            <span className="text-slate-500 uppercase font-bold text-[9px]">Validation Status</span>
            <span className={cn("font-bold mt-0.5", entity.isValidated ? "text-green-500" : "text-amber-500")}>
              {entity.isValidated ? 'Validated' : 'Requires Balancing'}
            </span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-slate-500 uppercase font-bold text-[9px]">Mapping Status</span>
            <span className={cn("font-bold mt-0.5", entity.isMapped ? "text-blue-500" : "text-slate-400")}>
              {entity.isMapped ? 'Completed' : 'Pending'}
            </span>
          </div>
        </div>
      )}
